...
export interface PostMovementInput {
  ...
  variantId: string;
  /**
   * Where the stock moves. Both optional and resolved through
   * `location.service.ts`: name a location to be exact, name only a branch to
   * get that branch's default, name neither to get the default branch's
   * default. A single-location business never supplies either.
   */
 ...
 * variant+location+lot on the row lock: the loser blocks until the winner
 * commits and then reads the committed value, rather than both proceeding from
...
const applyDelta = async (
  ...
  const rows = await tx.$queryRaw<Array<{ quantity: string | Prisma.Decimal }>>`
    INSERT INTO inventory (id, organization_id, variant_id, location_id, branch_id, lot_id, quantity, updated_at)
    VALUES (
      gen_random_uuid(),
  ...
  if (!row) {
    ...
    throw new Error(
      `Upsert of inventory for variant ${params.variantId} at location ${params.locationId} returned no row.`,
    );
  }
...
export const postMovement = async (
  ...
  const run = async (client: Prisma.TransactionClient) => {
    ...
    const { locationId, branchId } = await resolveLocation(
      input.organizationId,
      { locationId: input.locationId, branchId: input.branchId },
      client,
    );
    ...
    if (!input.allowNegative && balanceAfter.lessThan(0)) {
      ...
      throw new Error(
        `Movement would take variant ${input.variantId} at location ${locationId} to ${balanceAfter.toString()}. ` +
    ...
    return postMovement(
      {
        ...
        // The reversal must land exactly where the original did, so the
        // location is taken from the row rather than re-resolved.
